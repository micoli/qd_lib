...
class QDException extends Exception
{
	...
	//   if ($this->isErreur != -1 or is_array($this->getQDTrace())){
				...
				$string  = "\n<big><font color='red'>".($this->isErreur?"Erreur PHP ".$this->getLevelAsString()." ":"Exception levée ")."</font></big>";
...
{
	// En cas d'erreur que l'on souhaite récupèrer, on déclenche une exception
...
{
		// test de la présence de la mèthode getSummary, qui atteste de la présence de la classe QDException
		if (method_exists($exception, 'getSummary')){
			//echo "<pre>";print_r(debug_backtrace());
			// rajouter le test sur la variable blobal
			if (/* test de la variable globale */  0  )
		...
		else
		{
			...
			// autre que QDException n'a pas été récuperée
